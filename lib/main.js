(function() {
  'use strict';
  var C, DATOM, E, GUY, Interlex, _CRX, _X, alert, compose, copy_regex, debug, echo, equals, get_base_types, help, info, inspect, jump_symbol, lets, log, misfit, new_datom, plain, praise, rpr, urge, warn, whisper;
  ...
  Interlex = class Interlex {
    ...
    * walk(source) {
    ...
    step() {
      ...
      token = this._xxx({token, lexeme, match});
        ...
        case 'callme':
          ...
          switch (type_of_jump) {
            ...
            case 'popmode':
              ...
              break;
            default:
              throw new E.Interlex_internal_error('^interlex._xxx@1^', `unknown type_of_jump ${rpr(type_of_jump)} in lexeme ${rpr(lexeme)}`);
          }
          break;
        default:
          throw new E.Interlex_internal_error('^interlex._xxx@2^', `unknown type_of_jump in lexeme ${rpr(lexeme)}`);
      }
      return token;
    }
