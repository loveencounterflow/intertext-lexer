(function() {
  ...
  Interlex = class Interlex {
    ...
    _token_from_match(match) {
      var key, ref, stop, token_key, token_value, value, x;
      x = null;
      ref = match.groups;
      for (key in ref) {
        value = ref[key];
      ...
      stop = this.state.prv_last_idx + match[0].length;
      return this._new_token(token_key, token_value, this.state.prv_last_idx, stop, x);
    }
    ...
    * walk(source) {
      ...
      while (true) {
        ...
        if (this.state.prv_last_idx > max_index) {
          /* reached end */
          yield this._new_token('$eof', '', max_index + 1, max_index + 1);
          break;
        }
        match = source.match(pattern);
        if (match == null) {
          yield this._new_token('$error', '', this.state.prv_last_idx, this.state.prv_last_idx, {
            code: 'nomatch'
          });
          break;
        }
