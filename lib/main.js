(function() {
  'use strict';
  var C, DATOM, GUY, Interlex, _CRX, _X, alert, compose, copy_regex, debug, echo, equals, get_base_types, help, info, inspect, jump_symbol, lets, log, misfit, new_datom, plain, praise, rpr, urge, warn, whisper;
  ...
  Interlex = class Interlex {
    ...
    add_lexeme(cfg) {
      ...
      if (cfg.jump != null) {
        type_of_jump = this.types.isa.function(cfg.jump) ? 'function' : cfg.jump === jump_symbol ? 'pop' : 'push';
      } else {
        type_of_jump = 'null';
      }
    ...
    * walk(source) {
    ...
    step() {
      ...
      var after, before, center, divert, jump, left, lexeme, match, mid, ref, replacement_token, right, token;
      ...
      switch (lexeme.type_of_jump) {
        ...
        case 'pop':
          ...
          break;
        case 'function':
          if (this.types.isa.text((divert = lexeme.jump({
            token,
            lexeme,
            match,
            lexer: this
