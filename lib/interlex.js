...
/* NOTE avoiding shorthand for clarity */      ref = entry.lexemes;
    ...
    //---------------------------------------------------------------------------------------------------------
    _start(source = null) {
      var base, base1, base2, base3, base4, base5, entry, mode, ref, ref1, ref2, ref3, ref4, ref5;
      ...
      if ((base1 = this.state).offset == null) {
        base1.offset = {
          lnr: 1,
          x: 0
        };
      ...
      switch (this.cfg.state) {
        case 'keep':
          if ((base2 = this.state).stack == null) {
            base2.stack = [];
          }
          if ((base3 = this.state).mode == null) {
            base3.mode = (ref = this.base_mode) != null ? ref : null;
          }
          break;
        case 'reset':
          this.state.stack = [];
      ...
      if (this.cfg.split === 'lines') {
        if ((base4 = this.state).lnr1 == null) {
          base4.lnr1 = 0;
        }
        if ((base5 = this.state).eol == null) {
          base5.eol = '';
        }
      } else {
        this.state.lnr1 = 0;
    ...
    * _walk_file_lines(cfg) {
    ...
    //---------------------------------------------------------------------------------------------------------
    step() {
      var R, border, is_singleton_jump, nxt, prv, prv_mode, token;
      R = [];
      prv_mode = this.state.mode;
      token = this._step();
      //.......................................................................................................
      if (token != null) {
        if ((this.cfg.prepend !== '') || (this.cfg.append !== '')) {
          token = GUY.lft.lets(token, (token) => {
            return token.value = this.cfg.prepend + token.value + this.cfg.append;
          });
        }
        R.push(token);
        if (this.cfg.border_tokens && ((this.state.mode !== prv_mode) || (token.mode !== prv_mode))) {
          ...
          if (is_singleton_jump) {
            ...
            //...................................................................................................
            if (token.mode !== prv_mode) {
              R.unshift(lets(border, function(border) {
                return border.x1 = border.x2 = token.x1;
              }));
            } else if (token.mode === prv_mode) {
              R.push(lets(border, function(border) {
    ...
    //---------------------------------------------------------------------------------------------------------
    set_offset(cfg) {
      cfg = this.types.create.ilx_set_offset_cfg(cfg);
      this.state.offset = {...cfg};
      return null;
    }
