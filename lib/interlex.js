...
/* NOTE avoiding shorthand for clarity */      ref = entry.lexemes;
    ...
    * _walk_text_lines(cfg) {
    ...
    _call_jump_handler(lexeme, token, match) {
      var divert, jump, ref, replacement_token;
      divert = lexeme.jump({
        token,
        match,
        lexer: this
      ...
      return {token, jump, ...(this._parse_jump_cfg(jump))};
    }

    //---------------------------------------------------------------------------------------------------------
    _get_next_token(lexeme, token, match) {
      var jump, jump_action, jump_target, jump_time;
      /* TAINT code duplication */
      switch (lexeme.jump_action) {
        case 'nojump':
          ...
          break;
        case 'pushmode':
          token = this._push_mode(lexeme, token);
          break;
        case 'popmode':
          token = this._pop_mode(lexeme, token);
          break;
        case 'callme':
          ({token, jump, jump_action, jump_time, jump_target} = this._call_jump_handler(lexeme, token, match));
          switch (jump_action) {
            case 'nojump':
              null;
              break;
            case 'pushmode':
              this._push_mode(jump_target);
              break;
            case 'popmode':
              this._pop_mode();
              break;
            default:
              throw new E.Interlex_internal_error('^interlex._get_next_token@1^', `unknown jump_action ${rpr(jump_action)} in lexeme ${rpr(lexeme)}`);
          }
    ...
    _push_mode(lexeme, token) {
      ...
      this.state.mode = lexeme.jump_target;
      old_last_idx = this.state.pattern.lastIndex;
      this.state.pattern = this.registry[this.state.mode].pattern;
      this.state.pattern.lastIndex = old_last_idx;
      return lets(token, (token) => {
        token.jump = lexeme.jump_target;
        if (lexeme.jump_time === 'inclusive') {
